...
class cpu_model {
public:
    explicit cpu_model(const std::vector<instruction>& instructions)
        : instructions_ { instructions }
    {
    }

    double simulate(int unroll, bool print);
...
double cpu_model::simulate(int unroll, bool print)
{
    cycle_ = 1;
    pos_ = 0;
    unroll_ = unroll;
    ...
    const size_t print_width = 40;
    while (!done()) {
        const auto& poep_ins = get();
        int stall_cycles = 0;
        if (auto stall = check_change_use(poep_ins); stall.cycles) {
            if (print)
                std::cout << "\t; pOEP Change/use stall for " << stall.cycles << " cycles waiting for " << stall.reg << "\n";
        ...
        if (is_branch(poep_ins.op())) {
            if (print) {
                std::cout << "\t; Assuming correctly predicated (taking 0 cycles)\n";
                std::cout << "\t" << with_width(poep_ins, print_width) << "\n";
            }
            continue;
        }
        ...
        if (soep_ins) {
            ...
            if (reason.empty()) {
                ...
                if (auto stall = check_change_use(*soep_ins); stall.cycles) {
                    if (print)
                        std::cout << "\t; sOEP Change/use stall for " << stall.cycles << " cycles waiting for " << stall.reg << "\n";
        ...
        if (soep_ins) {
            if (reason.empty()) {
                ...
                if (print)
                    std::cout << "\t" << with_width(*soep_ins, print_width) << "; sOEP\n";
                ++pos_;
                update_register_change(*soep_ins);
            } else {
                if (print)
                    std::cout << "\t; sOEP idle because " << reason << "\n"; 
            }
        }
        cycle_ += icycles;
    }
    if (print) {
        std::cout << "\n\n";
        std::cout << (cycle_ - 1) << " cycles";
        if (unroll > 0)
            std::cout << " " << (static_cast<double>(cycle_ - 1) / (unroll + 1)) << " per iteration";
        std::cout << "\n";
    }
    return static_cast<double>(cycle_ - 1) / (unroll + 1);
}

void cpu_model::update_register_change(const instruction& i)
{
...
std::string cpu_model::soep_ok(const instruction& p, const instruction& s) const
{
    ...
    // 10.1.3 Dispatch Test 3: Allowable Effective Addressing Mode in the sOEP -- TODO
...
cpu_model::change_use_stall cpu_model::check_change_use(const instruction& i) const
{
...
cpu_model::change_use_stall cpu_model::check_change_use(const ea& e) const
{
...
cpu_model::change_use_stall cpu_model::calc_stall(const eareg& r, int cycles) const
{
...
int main(int argc, char* argv[])
{
    try {
        if (argc != 2)
            throw std::runtime_error { "Usage: " + std::string { argv[0] } + " source" };
